...
const pricingData = [
  {
    ...
    price: '$49',
    perks: 'CURATED GEAR &\nLIFESTYLE ITEMS',
    desc: 'Cancel anytime. Standard shipping included.'
  },
  {
    id: 2,
    tier: 'Pro',
    label: '.02',
    price: '$149',
    perks: 'EXCLUSIVE PARTS &\nLIMITED EDITIONS',
    desc: 'Cancel anytime. Priority shipping included.'
  }
];
...
const ParticleCard = ({
  ...
  const animateParticles = useCallback(() => {
    ...
    if (particlesRef.current.length > 0) return; // Already animating
    ...
    memoizedParticles.current.forEach((particle, index) => {
      const timeoutId = setTimeout(() => {
        ...
        gsap.fromTo(clone, { scale: 0, opacity: 0 }, { scale: 1, opacity: 1, duration: 1, ease: 'back.out(1.7)' });
  ...
  useEffect(() => {
    ...
    // Always animate stars regardless of hover
    animateParticles();
    ...
    <>
      ...
      <section id="pricing" className="rigid-grid" ref={gridRef}>
        ...
        {pricingData.map((tier, index) => {
          ...
          const CardContent = () => (
            <>
              <div className="tier-content-left">
                <div className="cell-header"><span className="cell-pill">{tier.label}</span></div>
                <h3 className="cell-title">{tier.tier} <br/><em>Subscription</em></h3>
                <div style={{marginTop: 'auto'}}>
                  <div className="cell-label">MONTHLY RESERVATION</div>
                  <div className="cell-value">{tier.price}</div>
                  <p className="cell-desc">{tier.desc}</p>
                </div>
              </div>
              <div className="tier-content-right">
